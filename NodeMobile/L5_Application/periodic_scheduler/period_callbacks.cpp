...
char a[1];
double s_latitude[200];
double s_longitude[200];
//int s_latitude[100];
//int s_longitude[100];
...
void period_10Hz(uint32_t count) {
	...
	if (xSemaphoreTake(start_sending, 0)) {
		...
		char* data = new char[20];
		if (!destiation_reached) {
			sprintf(data, "L%02dR%02dC%02dB%02dSPD%0.3f", my_left, my_right,
					center, rear, speed);
		} else {
			sprintf(data, "L%02dR%02dC%02dB%02dSTP%0.3f", my_left, my_right,
					center, rear, speed);
			destiation_reached = false;
		}
		u2.put(data, 0);
		...
		while (CAN_rx(can1, &can_msg, 0)) {
			...
			if (can_msg_hdr.mid == 0x20) {
				dbc_decode_MASTER_HB(&heartbeat, can_msg.data.bytes,
						&can_msg_hdr);
				//n is the number of the co-ordinates from the APP & last_flag is true if last is received from the app
				...
				if (heartbeat.MASTER_START_COORD == 1) {
					...
					if (counter < lat_counter) {
						...
						printf("%d) Co-ordinates=%lf,%lf \n", counter,
								s_latitude[counter], s_longitude[counter]);
			...
			if (can_msg_hdr.mid == 0x10) {
				dbc_decode_SENSOR_DATA(&sensordata, can_msg.data.bytes,
						&can_msg_hdr);
				my_left = sensordata.SENSOR_left_sensor;
				center = sensordata.SENSOR_middle_sensor;
				my_right = sensordata.SENSOR_right_sensor;
			...
			if (can_msg_hdr.mid == 0x30) {
				dbc_decode_MOTOR_STATUS(&motorstatus, can_msg.data.bytes,
						&can_msg_hdr);
				printf("\nMotor speed=%lf\n",
...
void period_100Hz(uint32_t count) {
	...
	if (stat[0] == '#' || stat[1] == '#') {
		...
		temporary_latitude_value = (37 + (lat1 * 0.1) + (lat2 * 0.01)
				+ (lat3 * 0.001) + (lat4 * 0.0001) + (lat5 * 0.00001)
				+ (lat6 * 0.000001));
		temporary_longitude_value = (121 + (lon1 * 0.1) + (lon2 * 0.01)
				+ (lon3 * 0.001) + (lon4 * 0.0001) + (lon5 * 0.00001)
				+ (lon6 * 0.000001));
		int repeat_counter;
		for (repeat_counter = 0; repeat_counter < lat_counter;
				repeat_counter++) {
			if ((s_latitude[repeat_counter] == temporary_latitude_value)
					&& (s_longitude[repeat_counter] == temporary_longitude_value)) {
				match_flag = 1;
			}
